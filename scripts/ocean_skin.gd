...
func _ready() -> void:
	...
	# дети готовы раньше родителей. Ждём кадр, иначе подменять нечего.
	await get_tree().process_frame
	apply()


## Вынесено отдельно, чтобы можно было позвать из редактора после правки цветов.
func apply() -> void:
	var surface := get_parent()
	if surface == null or shader == null:
		push_warning("OceanSkin: нет родителя или не задан шейдер.")
		return
	...
	# OceanSurface. Пусто — значит родитель не тот или он ещё не собрался.
	var mats: Array = []
	for prop in ["material", "far_material"]:
		...
		if mat != null:
			mats.append(mat)
	if mats.is_empty():
		push_warning("OceanSkin: у родителя нет материалов океана.")
		return
