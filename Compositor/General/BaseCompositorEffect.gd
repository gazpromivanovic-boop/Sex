...
func _notification(what):
	if what == NOTIFICATION_PREDELETE:
		...
		# Правка: отписка при удалении падала с "null instance". На стадии
		# PREDELETE экземпляр уже частично разобран, и обращаться к своему же
		# методу нельзя. Плюс bind() без аргументов даёт Callable, не равный
		# исходному, поэтому is_connected ниже всегда возвращал false и подписка
		# накапливалась. Везде используем сам метод, без bind.
		if Engine.is_editor_hint() and is_instance_valid(self):
			var fs = Engine.get_singleton("EditorInterface").get_resource_filesystem()
			if fs != null and fs.resources_reimported.is_connected(_reload):
				fs.resources_reimported.disconnect(_reload)
...
func _init():
	...
	if (Engine.is_editor_hint() and not Engine.get_singleton("EditorInterface").get_resource_filesystem().resources_reimported.is_connected(_reload)):
		Engine.get_singleton("EditorInterface").get_resource_filesystem().resources_reimported.connect(_reload)
